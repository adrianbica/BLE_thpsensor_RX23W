...
* associates a user callback function with the timer’s interrupt, and powers up and starts the timer
* @param[in] period_us Desired period in microseconds.  The range and resolution of the timer is determined by settings of the peripheral clock.
* The best pre-scaler for the CMT channel is chosen by the driver.\n
...
* @retval true  Successful; CMT initialized\n
* @retval false     No free CMT channels available, or invalid settings\n
* @details The R_CMT_CreateOneShotAssignChannelPriority assigns the desired CMT channel to the caller with desired interrupt priority level,
* and registers a user callback function to be called upon the compare match event.
* The CMT is configured to generate a compare match after the period specified in the call. The timer is shut down after a single compare match event.\n
...
* The CMT channel cannot be used again until it has been reopened with either the R_CMT_CreatePeriodic or the R_CMT_CreateOneShot function.
